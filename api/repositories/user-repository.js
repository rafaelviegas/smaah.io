...
exports.findByEmailAsync = async(email) =>{
    ...
    return await User.findOne({email}, 'name email roles');   
};

exports.findAsync = async(email, password) => {

    return await User.findOne({email, password}, 'name email roles');
};

exports.findByIdAsync = async(id) =>{

    return await User.findById(id, 'name email roles');
};
